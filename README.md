...
To generate a client with only a subset of operations (and their referenced schemas), use the object form:
...
This filters the source spec to only include the named operations (matched by `operationId`) and any schemas they reference. Schemas referenced transitively (e.g. a schema that `$ref`s another schema) are included automatically. This is useful for keeping generated clients small when you only need a few endpoints from a large API.
