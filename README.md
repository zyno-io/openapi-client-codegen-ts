...
The keys match the paths in `openapi-specs.json`; the values point to where the spec actually lives during development (e.g. a file generated by a locally running backend). When using watch mode, the generator watches the dev path instead of the committed path. It only copies the dev spec back to the committed path when their contents differ, so spec changes still show up in `git diff` without unnecessary file writes.

Each generated SDK directory contains a `.openapi-client-codegen.hash` file. It records the YAML's SHA-256 hash and output-affecting generator options; if it matches on a later run, generation is skipped.
